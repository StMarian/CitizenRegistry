namespace Citizens.Tests
{
    ...
    public class CitizenRegistryTests : TestsBase
    {
        ...
        {
            DateTime birthday = new DateTime(1991, 8, 24);
            var citizen = CitizenBuilder.NewWoman().WithDate(birthday).Build();
            registry.Register(citizen);

            DateTime dateStart = new DateTime(1899, 1, 31);
            string dateDiff = (birthday.Date - dateStart.Date).TotalDays.ToString();

            StringAssert.StartsWith(citizen.VatId, dateDiff);
        }
